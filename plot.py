...
bins = np.linspace(0.45, 0.55, 300)
for i in range(i_size):
    for j in range(j_size):
        
        #plt.legend(['Yhat'])
        #plt.hist(Y_test[:,i,j], bins, label = "Y_test", figure = fig)
        #plt.legend(['Y test'])
        plt.hist(Yhat[:,i,j], bins, label =" Yhat", figure = fig )
...
for i in range(i_size):
    for j in range(j_size):
        #plt.legend(['Yhat'])
        #plt.hist(Y_train[:167175,i,j], bins, label = "Y_train", figure = fig)
        #plt.legend(['Y test'])
        plt.hist(Yhat[:,i,j], bins, label =" Yhat", figure = fig)
        plt.legend(loc='upper right')
